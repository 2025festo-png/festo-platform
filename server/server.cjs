...
app.get('/api/events/:eventId/guest/:name', async (req, res) => {
    try {
        ...
        // Kërkimi krahason emrat pa parasysh shkronjat e mëdha/vogla dhe hapësirat
        const guest = guests.find(g => g.name && g.name.trim().toLowerCase() === decodedName);

        if (!guest) {
            return res.status(404).json({ 
                error: `I ftuari "${req.params.name}" nuk u gjet në listë.`
            });
        }
...
app.get('/api/events/:eventId/media', async (req, res) => {
    try {
        const { eventId } = req.params;
        const result = await pool.query('SELECT * FROM media WHERE event_id = $1 ORDER BY created_at DESC', [eventId]);
...
app.post('/api/events/:eventId/memory', async (req, res) => {
    try {
        ...
        res.status(201).json({ id: result.rows[0].id, name, text, table: table || 1, timestamp: new Date().toISOString() });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
app.get('/api/events/:eventId/memories', async (req, res) => {
    try {
        const { eventId } = req.params;
        const result = await pool.query('SELECT * FROM memories WHERE event_id = $1 ORDER BY timestamp DESC', [eventId]);
        res.json(result.rows);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// ============================================================
// MERK MEDIAT (PËR FAQEN E EVENTIT)
// ============================================================
app.get('/api/events/:eventId/media', async (req, res) => {
    try {
        const { eventId } = req.params;
...
// ============================================================
// MERK KUJTIMET (PËR FAQEN E EVENTIT)
// ============================================================
app.get('/api/events/:eventId/memories', async (req, res) => {
    try {
        const { eventId } = req.params;
